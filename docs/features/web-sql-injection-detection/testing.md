...
## Measured recall (P8, 2026-09-07)

On the captured corpus — 37 real SQLi strings fired through nginx over a real HTTP round-trip
(`web_sql_injection_captured_access.log`, made by `scripts/capture_web_attack_corpus.py`):

| | Precision | Recall | F1 | Notes |
|---|---|---|---|---|
| Static path, model off | **1.00** | **0.89** | 0.94 | up from 0.65 before the P8 rule additions |

The 5 residual misses — subquery booleans, `ASCII(SUBSTRING(`, `RLIKE (SELECT CASE`, hex
tautology — are borderline by design and route to the model tier. Precision is 0 false positives
across the benign corpus **and** 400 lines of real internet scanner traffic measured locally.
Numbers are regenerated by `tests/e2e/test_corpus_metrics_gate.py` into `out/metrics/`.

## Cases covered

**Pattern table** (`test_sql_injection_pattern_rules.py`)

- one positive per class: tautology (five shapes incl. parenthesised), union (three),
  error-based (three), stacked (two), blind (two)
