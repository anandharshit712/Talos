...
### 16.13 Revision 1.13 — IDOR detection, P6.2 (2026-09-04)

| Change | Where | Why |
|---|---|---|
| **The endpoint feature compares a template, not the path** | §7.4 | `target.endpoint` carries the object id, so `/api/orders/1002` and `/api/orders/1003` are different endpoints. Comparing the raw path makes every object access a novel endpoint forever — `novel_endpoint` becomes a constant, and the bounded `endpoints` map grows one entry per object until it evicts the real endpoints. `endpoint_template()` strips the trailing id; the baseliner and the scorer both use it, and `Scope.affected_endpoints` reports it. Found by the discriminator test firing on a legitimate user opening one new record. |
| **`sequential_run` reads the event window, not the baseline; weights are configuration** | §7.4, §10 | Once a walk begins, each id widens the learned range, so a baseline-only view loses the run after the first id. `outside_range` catches the initial jump and the window carries the rest. The four weights, the floor, the judge weight and the rate saturation live in `talos.detection.idor` — no threshold is a module-level literal (standards §2.3), and P8 moves them. |
| **A disagreeing judge caps the finding; it does not veto it** | §7.4, §8.3 | The LLD's `blend()` was unspecified for `is_idor: false`. Discarding the deterministic score would let one model reply silence a measured deviation. The score is capped at `low_score_floor` instead, which turns a finding into a lead. |
| **Cold start returns a non-firing verdict** | §7.4, §4.3 | `attack_detected=False` with `immature_confidence` and evidence naming the shortfall. The aggregator already drops non-firing verdicts, so an unfamiliar account produces no incident while the record still shows the detector looked — which is the honest answer, and cheaper than a second "informational verdict" path. |
| **Score before learning; learn only from clean traffic** | §7.4, §3 | Two ordering rules in the sub-agent. Learning first puts the current id inside the learned range, so `outside_range` is false by construction and the detector can never fire on the tripping event. Learning from flagged traffic teaches the baseline that walking the id space is normal for that account — a detector training itself to stop detecting. Both have tests that fail if the order is swapped. |
| **`access_baseliner` has no routing entry** | §7.4, §8.2 | The LLD gave it the `long_context` tier. Folding an access into a baseline is arithmetic, and a per-event network call for it would sit on the per-event hot path for a decision a model cannot improve. The entry is removed and the tier has no consumer this cycle. |
| **Routing repaired before the gate** | §8.2 | `meta/llama-3.1-8b-instruct` (EOL 2026-08-26) and `nvidia/nemotron-3-nano-30b-a3b` (EOL 2026-09-01) return 410; `mistral-large-2512` returns 403 for this subscription tier. The `nano` tier's primary moves to Groq — NVIDIA serves this account nothing at that size any more — and the `heavy` fallback becomes Groq's 120B. 7/7 routed models probed live. |

### 16.14 Revision 1.14 — the output surface, P7 (2026-09-04)

| Change | Where | Why |
|---|---|---|
| **One pipeline per process, built at startup** | §2.1, §4.2 | `create_app` shares a single orchestrator across requests. This is correctness, not performance: the event window and the duplicate filter are per-process state, so a per-request pipeline would forget the burst it is halfway through detecting and re-alert on every event of it. The consequence is that the API is **single-process** — two workers each hold half the events, so a burst split between them may trip neither threshold. Recorded as a deployment constraint with a named fix (a shared window) rather than left implicit. |
| **`POST /events` answers `204`, not a report saying "no attack"** | §2.1, §4.3 | Most events are not attacks. A body carrying `attack_detected: false` invites a caller to read absence of evidence as a verdict, and the orchestrator's contract is already that `None` means nothing fired rather than an empty incident. |
| **A storage failure is `503`, not `200`** | §11 | Fail-safe for reporting. An event analysed but not recorded has not been accounted for; answering `200` would tell the caller otherwise and lose the incident silently. |
| **`GET /healthz` does a database round-trip** | §2.1 | A process that answers `200` while unable to record an incident is the exact failure a health check exists to catch, so liveness alone would be worse than nothing. |
| **`TalosState` is typed and lives in `report_routes`** | §2.1 tree | `app.state.orchestrator` is `Any`, so a typo in a route is a 500 at runtime instead of a mypy error. It sits beside the routes because the factory imports them; the reverse would be a cycle, and a third module for two declarations would be worse than either. |
| **The suppression filter is a TTL map in event time** | §4.3, §10 | It was a plain dict with a hard cap that **cleared itself** when full. On a long-running server a burst of unrelated signatures therefore wipes the memory of an attack still in progress, and every one of those attacks re-alerts. Now `suppression_ttl_seconds` (event time, matching the event window so a replay behaves like a live stream) with `max_tracked_incidents` evicting oldest-first. |
...
| **`union_select` now requires a column list** | §7.1 | An adversarial prose probe showed `\bunion\s+select\b` firing at 0.95 on `how do I write a UNION SELECT across two tables` — vocabulary in a sentence, the exact failure the table's own note warns against. The rule now requires a star, a number, a quote, a function call, a comma-list, or a FROM after SELECT. Lossless: captured-payload recall held at 0.89, because every real UNION injection carries a column list. |
| **Calibration measured as unmeasurable; `calibration:` left empty on purpose** | §9 | The corpus produces zero wrong verdicts, so every confidence band reads 1.00 and NFR-3's "0.9 → 90% correct" cannot be evaluated. The reading: the static confidence is a conservative strength signal, never overconfident in the safe direction. An adversarial prose probe (`web_attack_vocabulary_in_prose.log`, pinned by a test) found the one overconfident regime — attack vocabulary in free-text fields — of which four cases are the XSS surface (a script tag in a parameter is dangerous input) and three are field-type ambiguity an access log cannot resolve. Fabricating a curve from either the degenerate or the OOD data would mis-tune a detector that is already right in-distribution, so the config stays empty with the trigger recorded. Full write-up in `docs/operations/Talos_Evaluation_Results.md`. |
